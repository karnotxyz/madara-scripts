...
const CONFIG = {
  ...
  NODE_URL: 'https://madara-test.karnot.xyz/',
  ACCOUNT_ADDRESS: '0x5beeb0d4a821f90bf209b0404e328670638ae0dad167eed788b733a6df769c5',
  PRIVATE_KEY: '',
  CHAIN_ID: '1',
  TO_ADDRESS: '0x030808B129E9B824C40beD2e0A2dEcAc1a30A03D94A3981621EBfe78bA82B061',
  MAX_FEE: '1293261266510',
  TRANSFER_COUNT: 5000,
  AMOUNT: '1000000000000000000' // Add your amount here
};
...
async function transfer_internal(from_account, to_address, amount, nonce) {
  const contract = new starknet.Contract(ERC20.abi, CONFIG.ETH_ADDRESS, provider);
  ...
  let hash = await from_account.execute(result, {
      nonce,
      maxFee: CONFIG.MAX_FEE,
  });
...
async function processTransfers() {
  ...
  for (let i = 0; i < CONFIG.TRANSFER_COUNT; i++) {
      console.log('Transfer number - ', i, 'Nonce - ', nonce);
      let transaction_hash = await transfer_internal(
          account,
          CONFIG.TO_ADDRESS,
          CONFIG.AMOUNT,
          nonce
  ...
  const promises = hashes.map(hash => provider.waitForTransaction(hash));
  const results = await Promise.all(promises);

  // Log results
  results.forEach((result, index) => {
      console.log(`Transaction receipt ${index + 1}:`, result);
  });
...
async function main() {
  try {
      ...
      const results = await processTransfers();
